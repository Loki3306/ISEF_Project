    ...
    with torch.no_grad():
        for i in range(len(X)):
            ...
            while start + window_samples <= x_np.shape[1]:
                ...
                x_chunk_10s = torch.FloatTensor(x_np[:, start:end]).unsqueeze(0).to(device)
                if zero_eeg:
                    x_chunk_10s = torch.zeros_like(x_chunk_10s)
...
def train_matchnet_loso(channels, lowcut, highcut, batch_size=128, num_workers=2, subjects_to_run=None):
    torch.backends.cudnn.benchmark = True
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} | SA2MatchNet Diagnostic | Channels: {channels}")
    ...
    for held_out_path, train_paths in folds:
        ...
        # Model
        model = SA2MatchNet(eeg_channels=len(channels), audio_channels=NUM_BANDS, seq_len=int(TRAIN_WINDOW_SEC*FS)).to(device)
        
        # Log Architecture Diagnostics
        print("\n--- SA-2 ARCHITECTURE DIAGNOSTICS ---")
        eeg_params = sum(p.numel() for p in model.eeg_encoder.parameters() if p.requires_grad)
        audio_params = sum(p.numel() for p in model.audio_encoder.parameters() if p.requires_grad)
        print(f"EEG Encoder Parameters: {eeg_params:,}")
        print(f"Audio Encoder Parameters: {audio_params:,}")
        print(f"Total Trainable Parameters: {eeg_params + audio_params:,}")
        
        # Test a forward pass for shapes
        with torch.no_grad():
            dummy_eeg, dummy_a, dummy_b = next(iter(train_loader))
            dummy_eeg = dummy_eeg.to(device)
            dummy_a = dummy_a.to(device)
            dummy_b = dummy_b.to(device)
            z_eeg, z_a, z_b = model(dummy_eeg[:2], dummy_a[:2], dummy_b[:2])
            print(f"EEG Output Shape: {z_eeg.shape}")
            print(f"Audio Output Shape: {z_a.shape}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train SA-2 Diagnostic")
    ...
    train_matchnet_loso(args.channels, args.lowcut, args.highcut, args.batch_size, args.num_workers, args.subjects)
