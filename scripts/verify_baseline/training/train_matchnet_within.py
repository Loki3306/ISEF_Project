...
def train_matchnet_within(eeg_model="eegnet", channels=[0, 33, 6, 41, 22, 59, 15, 52], lowcut=1.0, highcut=6.0, batch_size=512, num_workers=4, subjects_to_run=None, loss_type="contrastive", lambda_align=0.5, align_target=0.1):
    ...
    for p in all_paths:
        subject_key = str(p)
        print(f"\nEvaluating Within-Subject for: {p.stem}")
        ...
        with open(f"checkpoints/matchnet_within_{p.stem}_metrics.json", "w") as f:
...
def main():
    ...
    train_matchnet_within(args.model, args.channels, args.lowcut, args.highcut, args.batch_size, args.num_workers, args.subjects, args.loss, args.lambda_align, args.align_target)
