...
def train_finetune_bn(eeg_model="eegnet", channels=[0, 33, 6, 41, 22, 59, 15, 52], lowcut=1.0, highcut=6.0, batch_size=512, num_workers=4, subjects_to_run=None, loss_type="contrastive", lambda_align=0.5, align_target=0.1, pretrained_dir="checkpoints"):
    ...
    for p in all_paths:
        ...
        tX, tYA, tYB = prepare_dataset(subject_examples[subject_key], channels, lowcut, highcut, p.stem, mapping, envelopes)
        ...
        X_tr_full = [tX[i] for i in train_idx]
        YA_tr_full = [tYA[i] for i in train_idx]
        YB_tr_full = [tYB[i] for i in train_idx]
        
        X_va_full = [tX[i] for i in val_idx]
        YA_va_full = [tYA[i] for i in val_idx]
        YB_va_full = [tYB[i] for i in val_idx]
        
        X_te_full = [tX[i] for i in test_idx]
        YA_te_full = [tYA[i] for i in test_idx]
        YB_te_full = [tYB[i] for i in test_idx]
        
        if p == all_paths[0]:  # Only print for the first subject
        
            if len(train_idx) > 0:
                vc_x, vc_ya, vc_yb = chunk_trial(X_tr_full[0], YA_tr_full[0], YB_tr_full[0], TRAIN_WINDOW_SEC, TRAIN_HOP_SEC)
            ...
            print("--- 2. EXECUTING FULL WITHIN-SUBJECT RUN ---")
        ...
        for epoch in range(25):
            ...
            print(f"  Epoch {epoch+1:02d}/25 | Loss: {avg_loss:.4f} (sA: {avg_sa:.3f}, sB: {avg_sb:.3f}) | Val Acc: {val_acc*100:.2f}% | Patience: {epochs_no_improve}/{patience}")
        ...
        final_path = f"checkpoints/matchnet_finetune_bn_{p.stem}_final.pth"
        torch.save(model.state_dict(), final_path)
        
        model.load_state_dict(best_weights)
        best_path = f"checkpoints/matchnet_finetune_bn_{p.stem}_best.pth"
        ...
        with open(f"checkpoints/matchnet_finetune_bn_{p.stem}_metrics.json", "w") as f:
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune Contrastive MatchNet (BN layers only)")
    ...
    parser.add_argument("--subjects", type=str, nargs='+', default=None, help="Specific subjects to run (e.g. S1_data_preproc)")
    ...
    parser.add_argument("--pretrained_dir", type=str, default="checkpoints", help="Directory containing the pretrained LOSO weights")
    args = parser.parse_args()
    
    train_finetune_bn(args.model, args.channels, args.lowcut, args.highcut, args.batch_size, args.num_workers, args.subjects, args.loss, args.lambda_align, args.align_target, args.pretrained_dir)
