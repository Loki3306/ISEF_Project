...
def prepare_dataset(examples, lowcut, highcut, subject_id, mapping, envelopes):
    X = []
    Y_A = []
    Y_B = []
    ...
    for i, ex in enumerate(examples):
        eeg = ex.eeg.T  # Shape: [Channels, Time]
...
def evaluate_model(model, X, Y_A, Y_B, device, window_sec=10, zero_eeg=False, shuffle_labels=False, metric="cosine", shuffle_eeg_time=False, shuffle_audio_time=False, permute_spatial=False, swap_ab=False, record_csv=False, csv_path=None, subject_id=""):
    """
    ...
    if record_csv and csv_path and not os.path.exists(csv_path):
        with open(csv_path, "w") as f:
            f.write("subject,trial,window,sim_A,sim_B,prediction,label,correct\n")
    ...
    with torch.no_grad():
        for i in range(len(X)):
            ...
            while start + window_samples <= x_np.shape[1]:
                ...
                if record_csv and csv_path:
                    with open(csv_path, "a") as f:
                        f.write(f"{subject_id},{i},{n_total},{sim_a:.4f},{sim_b:.4f},{pred},{label},{int(is_correct)}\n")
                    
                n_total += 1
                start += window_samples
                
    return n_correct, n_total, np.mean(all_sim_a), np.mean(all_sim_b), np.mean(all_margins)

def train_matchnet_64ch_loso(eeg_model, lowcut, highcut, batch_size=128, num_workers=2, subjects_to_run=None, loss_type="contrastive", lambda_align=0.5, align_target=0.1):
    torch.backends.cudnn.benchmark = True
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} | MatchNet 64-Channel ({eeg_model})")
    ...
    for held_out_path, train_paths in folds:
        ...
        if held_out_key == str(folds[0][0]):  # Only print for the first fold
            ...
            if len(train_paths) > 0:
                v_X, v_YA, v_YB = prepare_dataset(subject_examples[str(train_paths[0])], lowcut, highcut, train_paths[0].stem, mapping, envelopes)
        ...
        for p in train_paths:
            ...
            tX, tYA, tYB = prepare_dataset(subject_examples[str(p)], lowcut, highcut, p.stem, mapping, envelopes)
        ...
        X_te_full, YA_te_full, YB_te_full = prepare_dataset(test_exs, lowcut, highcut, held_out_path.stem, mapping, envelopes)
        ...
        model = ContrastiveMatchNet(eeg_model_type=eeg_model, eeg_channels=num_channels, audio_channels=NUM_BANDS, latent_dim=64).to(device)
        ...
        for epoch in range(100):
            ...
            nc_va, nt_va, _, _, _ = evaluate_model(model, X_va_full, YA_va_full, YB_va_full, device, window_sec=10)
        ...
        csv_path = f"checkpoints/matchnet_64ch_decisions_fold_{held_out_path.stem}.csv"
        nc_norm, nt_norm, mean_sa, mean_sb, mean_margin = evaluate_model(model, X_te_full, YA_te_full, YB_te_full, device, window_sec=w_sec, metric="pearson", record_csv=True, csv_path=csv_path, subject_id=held_out_path.stem)
        acc_norm = nc_norm / max(nt_norm, 1)
        
        nc_zero, _, _, _, _ = evaluate_model(model, X_te_full, YA_te_full, YB_te_full, device, window_sec=w_sec, zero_eeg=True, metric="pearson")
        acc_zero = nc_zero / max(nt_norm, 1)
        
        nc_shuf, _, _, _, _ = evaluate_model(model, X_te_full, YA_te_full, YB_te_full, device, window_sec=w_sec, shuffle_labels=True, metric="pearson")
        acc_shuf = nc_shuf / max(nt_norm, 1)
        
        nc_shuf_eeg_time, _, _, _, _ = evaluate_model(model, X_te_full, YA_te_full, YB_te_full, device, window_sec=w_sec, shuffle_eeg_time=True, metric="pearson")
        acc_shuf_eeg_time = nc_shuf_eeg_time / max(nt_norm, 1)
        
        nc_shuf_audio_time, _, _, _, _ = evaluate_model(model, X_te_full, YA_te_full, YB_te_full, device, window_sec=w_sec, shuffle_audio_time=True, metric="pearson")
        acc_shuf_audio_time = nc_shuf_audio_time / max(nt_norm, 1)
        
        nc_perm_spatial, _, _, _, _ = evaluate_model(model, X_te_full, YA_te_full, YB_te_full, device, window_sec=w_sec, permute_spatial=True, metric="pearson")
        acc_perm_spatial = nc_perm_spatial / max(nt_norm, 1)
        
        nc_swap, _, _, _, _ = evaluate_model(model, X_te_full, YA_te_full, YB_te_full, device, window_sec=w_sec, swap_ab=True, metric="pearson")
        acc_swap = nc_swap / max(nt_norm, 1)
        
        print(f"    -> Window {w_sec:2d}s | Normal: {acc_norm*100:.2f}% | Decisions: {nt_norm}")
        print(f"    -> Telemetry  | Mean Sim_A: {mean_sa:.4f} | Mean Sim_B: {mean_sb:.4f} | Mean Margin: {mean_margin:.4f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train 64-Channel Contrastive MatchNet")
    ...
    train_matchnet_64ch_loso(args.model, args.lowcut, args.highcut, args.batch_size, args.num_workers, args.subjects, args.loss, args.lambda_align, args.align_target)
